...
## Fix v0.3.1
Questa versione corregge il problema per cui, dopo la prima sezione, alcune aree potevano restare completamente nere.
La causa era il sistema di `reveal on scroll`: gli elementi partivano con `opacity: 0` e, se lo script di animazione non si inizializzava correttamente o veniva rallentato/bloccato, non tornavano visibili.

La v0.3.1 usa **progressive enhancement**:
- il contenuto è visibile di default;
- viene nascosto solo quando l'animazione è stata agganciata correttamente;
- esiste un fallback su scroll;
- esiste un fail-safe finale se JavaScript o una dipendenza opzionale falliscono;
- su `prefers-reduced-motion` il contenuto resta sempre visibile.
