(() => {
  ...
  function initGlobalActions(){
    if(document.documentElement.dataset.finalActions==='1') return;
    document.documentElement.dataset.finalActions='1';

    document.addEventListener('click',e=>{
      const add=e.target.closest?.('#addInvestorBtn');
      if(add){
        e.preventDefault();
        e.stopPropagation();
      ...
      if(edit){
        e.preventDefault();
        const inv=byId(edit.dataset.editId);
        if(inv) openInvestorModal(inv);
        return;
      }
      ...
      if(del){
        e.preventDefault();
        const inv=byId(del.dataset.deleteId);
        if(inv) removeInvestor(inv);
        return;
      }
  ...
  // Exposed only as a fallback for the static HTML button.
  window.DronepadOpenInvestorModal=()=>openInvestorModal();
  ...
  function initPipeline(){
    ...
    ['#searchInput','#stageFilter','#typeFilter','#priorityFilter'].forEach(selector=>{
      const el=$(selector);
      if(!el || el.dataset.boundFinal==='1') return;
      el.dataset.boundFinal='1';
      el.addEventListener(el.tagName==='INPUT'?'input':'change',renderPipeline);
    });

    const form=$('#investorForm');
    if(form && form.dataset.boundFinal!=='1'){
      form.dataset.boundFinal='1';
      form.addEventListener('submit',async e=>{
        e.preventDefault();
        try{
          await saveInvestorForm(e.currentTarget);
          closeDialog('investorDialog');
          renderPipeline();
        }catch(err){
          toast(authErrorText(err),'error');
        }
  ...
  function renderDetail(){
    ...
    $('#editInvestorBtn').onclick=()=>{fillDetailForm(inv);openDialog('investorDialog');};
