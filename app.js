...
function openAuthDialog(){
  if(!window.DronepadCloud?.configured){
    showToast(currentLang==='it'?'Workspace privato attivo su questo dispositivo.':'Private workspace active on this device.');
    return;
  }
  const d=$('#authDialog');
...
function refreshCloudUI(){
  ...
  statusText.textContent=cloudMode?'CLOUD SYNC':'PRIVATE WORKSPACE';
  mini.querySelector('span').textContent=cloudMode?'CLOUD / SYNC ON':'PRIVATE STACK';
  storage.querySelector('span').textContent=cloudMode?(currentLang==='it'?'Storage privato sincronizzato.':'Private storage synced.'):tr('ecosystem.storageLocal');
