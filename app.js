const STORAGE_KEYS = {
  ...
  investors: 'dronepadHQ_investors_v3',
  tasks: 'dronepadHQ_tasks_v3',
  prototype: 'dronepadHQ_prototype_v3'
};
...
const i18n = {
  it: {
    title: 'Dronepad HQ — Mission Control v0.3',
    brandSub: 'Mission Control · v0.3',
    nav: {hero:'Inizio', overview:'Overview', platform:'Piattaforma', prototype:'Prototipo', investors:'Investitori', ecosystem:'Ecosistema', tasks:'Azioni'},
    top: {live:'PROGETTO LIVE', export:'Esporta JSON', progress:'Journey', scroll:'Scorri per esplorare'},
    hero: {
      eyebrow: 'DALL\'IP PROTETTA ALL\'INFRASTRUTTURA OPERATIVA',
      title: 'COSTRUIRE IL PRIMO<br><span class="orange">DRONEPAD REALE.</span>',
    ...
    overview: {
      ...
      title: 'DRONEPAD,<br>IN UNA SOLA VISTA.',
    ...
    platform: {
      ...
      title: 'DA MVP LOCALE A<br>INFRASTRUTTURA PRIVATA.',
      body: 'Questa è la trasformazione richiesta per rendere Dronepad HQ un vero strumento di lavoro condiviso, sicuro e accessibile ovunque.',
      todoKicker: 'IMPLEMENTAZIONE',
      todoTitle: 'Priorità tecniche immediate',
      ...
      stackTitle: 'Una piattaforma reale, non un semplice sito.',
      stackBody: 'Repository privato, autenticazione, database, storage riservato e deploy controllato diventano il sistema operativo digitale del progetto.'
    },
    prototype: {
      eyebrow: 'PROTOTYPE READINESS',
      title: 'TRASFORMARE IL CONCEPT<br>IN UN BUILD PLAN.',
      ...
      visualNote: 'Visual concettuale: le componenti andranno sostituite progressivamente con la BOM e l\'engineering reale del prototipo.'
    },
    investors: {
      eyebrow: 'CRM CAPITALI & NETWORK',
      title: 'PIPELINE<br>INVESTITORI.',
    ...
    ecosystem: {
      ...
      title: 'CAPITALE È SOLO<br>UNA PARTE.',
      ...
      dataKicker:'DATA ROOM', dataTitle:'Unica fonte di verità', upload:'Carica file', storageLocal:'Modalità locale: upload cloud disattivato.'
    },
    tasks: {
      eyebrow:'EXECUTION BOARD', title:'LE PROSSIME<br>AZIONI.', body:'Il progetto avanza quando ogni open point ha un owner, una scadenza e un output.', add:'+ Aggiungi task',
    ...
    auth: {title:'Accedi a Dronepad HQ', body:'Quando Supabase è configurato, questo accesso protegge CRM, task e data room.', signin:'Accedi', signout:'Esci'},
    misc: {
  ...
  en: {
    title: 'Dronepad HQ — Mission Control v0.3',
    brandSub: 'Mission Control · v0.3',
    nav: {hero:'Start', overview:'Overview', platform:'Platform', prototype:'Prototype', investors:'Investors', ecosystem:'Ecosystem', tasks:'Actions'},
    top: {live:'PROJECT LIVE', export:'Export JSON', progress:'Journey', scroll:'Scroll to explore'},
    hero: {
      eyebrow: 'FROM PROTECTED IP TO OPERATING INFRASTRUCTURE',
      title: 'BUILD THE FIRST<br><span class="orange">REAL DRONEPAD.</span>',
    ...
    platform: {
      ...
      title: 'FROM LOCAL MVP TO<br>PRIVATE INFRASTRUCTURE.',
      body: 'This is the required transformation to turn Dronepad HQ into a true shared workspace: secure, connected and accessible from anywhere.',
      todoKicker: 'IMPLEMENTATION',
      todoTitle: 'Immediate technical priorities',
      secureKicker: 'SECURITY BY DESIGN',
      ...
      stackTitle: 'A real platform, not just a website.',
      stackBody: 'Private repository, authentication, database, protected storage and controlled deployment become the digital operating system of the project.'
    },
    prototype: {
      eyebrow: 'PROTOTYPE READINESS',
      title: 'TURN THE CONCEPT<br>INTO A BUILD PLAN.',
      ...
      visualNote: 'Concept visual: components will progressively be replaced by the real prototype BOM and engineering.'
    },
    investors: {
      eyebrow: 'CAPITAL & NETWORK CRM',
      title: 'INVESTOR<br>PIPELINE.',
    ...
    ecosystem: {
      ...
      title: 'CAPITAL IS ONLY<br>ONE PIECE.',
      ...
      dataKicker:'DATA ROOM', dataTitle:'Single source of truth', upload:'Upload file', storageLocal:'Local mode: cloud upload disabled.'
    },
    tasks: {
      eyebrow:'EXECUTION BOARD', title:'THE NEXT<br>ACTIONS.', body:'The project moves forward when every open point has an owner, a deadline and an output.', add:'+ Add task',
...
const tData = {
  ...
  platformPriorities: {
    it: [
      ['Preparare repo privato','Separare il codice dalla futura area pubblica del progetto','NOW'],
      ['Migrare da localStorage a database','Passare a Supabase/PostgreSQL con struttura dati solida','NOW'],
      ['Attivare autenticazione','Login, ruoli e gestione accessi','NOW'],
      ['Connettere storage privato','Caricare file sensibili in area riservata','NEXT'],
      ['Creare data room condivisa','Documenti diligenti ordinati per workstream','NEXT']
    ],
    en: [
      ['Set up private repo','Separate the codebase from any public-facing project area','NOW'],
      ['Migrate from localStorage to database','Move to Supabase/PostgreSQL with a solid data model','NOW'],
      ['Enable authentication','Login, roles and access management','NOW'],
      ['Connect private storage','Upload sensitive files to a protected area','NEXT'],
      ['Build a shared data room','Diligence-ready documents organised by workstream','NEXT']
    ]
  },
  prototypeItems: {
    it: [
...
function renderArchitecture(){
  $('#architectureGrid').innerHTML = tData.architecture[currentLang].map(x=>`<article class="arch-step reveal"><div class="arch-node">${x[0]}</div><h4>${x[1]}</h4><p>${x[2]}</p></article>`).join('');
  $('#platformPriorities').innerHTML = tData.platformPriorities[currentLang].map((x,i)=>`<div class="path-row"><div class="path-index">0${i+1}</div><div><b>${x[0]}</b><small>${x[1]}</small></div><span class="tag ${x[2]==='NOW'?'accent':'neutral'}">${x[2]}</span></div>`).join('');
}
...
function taskFromDb(r){return {id:r.id,title:{it:r.title_it||'',en:r.title_en||r.title_it||''},stream:r.stream,status:r.status};}
...
function renderPartners(){$('#partnerGrid').innerHTML = tData.partners[currentLang].map((x,i)=>`<article class="partner-card"><span class="kicker">0${i+1}</span><b>${x[0]}</b><p>${x[1]}</p></article>`).join('');}
function renderMarket(){$('#marketGrid').innerHTML = tData.marketCards[currentLang].map(x=>`<article class="market-card"><span class="kicker">${x[0]}</span><b>${x[1]}</b><p>${x[2]}</p><ul>${x[3].map(y=>`<li>${y}</li>`).join('')}</ul></article>`).join('');}
function renderRegs(){$('#regList').innerHTML = tData.regs[currentLang].map((x,i)=>`<div class="reg-row"><div class="num">0${i+1}</div><div><b>${x[0]}</b><p>${x[1]}</p></div><span class="tag ${x[2]==='Open'?'warn':'neutral'}">${x[2]==='Open'?tr('misc.open'):tr('misc.later')}</span></div>`).join('');}
...
function renderCritical(){$('#criticalPath').innerHTML = tData.critical[currentLang].map((x,i)=>`<div class="path-row"><div class="path-index">0${i+1}</div><div><b>${x[0]}</b><small>${x[1]}</small></div><span class="tag ${x[2]==='NOW'?'accent':'neutral'}">${x[2]}</span></div>`).join('');}
function renderRoadmap(){$('#roadmap').innerHTML = tData.roadmap[currentLang].map(x=>`<div class="gate"><div class="gate-dot"></div><div><b>${x[0]}</b><span>${x[1]}</span></div></div>`).join('');}
...
function exportData(){
  const payload={exportedAt:new Date().toISOString(),language:currentLang,project:'Dronepad HQ',version:'0.3',cloudMode,investors,tasks,prototypeChecklist:tData.prototypeItems[currentLang].map((x,i)=>({item:x[0],note:x[1],done:protoState[i]})),platformArchitecture:tData.architecture[currentLang],documents:uploadedDocuments.map(({name,mime_type,size_bytes,category,status})=>({name,mime_type,size_bytes,category,status}))};
...
async function addInvestorFromForm(fd){
  const item={id:Date.now(),name:fd.get('name'),type:fd.get('type'),fit:{it:fd.get('fit'),en:fd.get('fit')},verification:'To verify',stage:fd.get('stage'),next:{it:fd.get('next'),en:fd.get('next')},source:{it:tr('misc.sourceManual'),en:i18n.en.misc.sourceManual}};
...
async function addTaskFromForm(fd){
  const item={id:Date.now(),title:{it:fd.get('title'),en:fd.get('title')},stream:fd.get('stream'),status:fd.get('status')};
...
function bindUI(){
  ...
  $('#documentUpload').addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;if(!cloudMode){showToast(currentLang==='it'?'Configura Supabase per usare lo storage privato.':'Configure Supabase to use private storage.');e.target.value='';return;}try{const doc=await window.DronepadCloud.uploadDocument(file);uploadedDocuments.push(doc);renderUploadedFiles();showToast(currentLang==='it'?'File caricato nella Data Room.':'File uploaded to the Data Room.');}catch(err){showToast(cloudErrorText(err));}finally{e.target.value='';}});
}

function openAuthDialog(){
  const d=$('#authDialog');
  $('#authError').textContent='';
  if(!window.DronepadCloud?.configured){$('#authIntro').textContent=currentLang==='it'?'Supabase non è ancora configurato: il pacchetto sta funzionando in modalità locale. Segui il README per attivare login, database e storage privato.':'Supabase is not configured yet: the package is running in local mode. Follow the README to enable login, database and private storage.';}
  else $('#authIntro').textContent=tr('auth.body');
  d.showModal();
}
...
function refreshCloudUI(){
  ...
  statusBtn.classList.toggle('connected',cloudMode);mini.classList.toggle('connected',cloudMode);storage.classList.toggle('connected',cloudMode);
  statusText.textContent=cloudMode?'CLOUD SYNC':(cloud?.configured?'AUTH READY':'LOCAL MODE');
  mini.querySelector('span').textContent=cloudMode?'SUPABASE / SYNC ON':(cloud?.configured?'SUPABASE / LOGIN READY':'LOCAL DEMO');
  storage.querySelector('span').textContent=cloudMode?(currentLang==='it'?'Storage privato attivo.':'Private storage active.'):tr('ecosystem.storageLocal');
  signOut.style.display=currentUser?'inline-flex':'none';
  initial.textContent=currentUser?.email?.[0]?.toUpperCase()||'A';
}
